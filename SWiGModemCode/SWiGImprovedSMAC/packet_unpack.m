...
function [pkt err] = packet_unpack(data)
    ...
    % 1 bit: Mobility Flag.
    pkt.mobility = uint64(bitand(bitshift(bin, -59), 1));

    % 1 bit: Schedule Flag.
    pkt.schedule = uint64(bitand(bitshift(bin, -58), 1));

    % 1 bit: Tx/Rx Flag.
    pkt.tx_rx = uint64(bitand(bitshift(bin, -57), 1));

    % 1 bit: Forwarding Capability.
    pkt.forward = uint64(bitand(bitshift(bin, -56), 1));

    % 8 bits: Class User Identifier.
    pkt.class_id = uint64(bitand(bitshift(bin, -48), 2^8 - 1));
    ...
    if (pkt.schedule == 1)
        ...
        pkt.reserv_repeat = uint64(bitand(bitshift(bin, -41), 1));
...
%     end
    ...
    if (app_data_len == 26)
        pkt.app_data = uint64(dec2hex(pkt.app_data(:), 7));
    else
        pkt.app_data = uint64(dec2hex(pkt.app_data(:), 9));
    end
    ...
    % Check CRC.
    my_crc = crc(data(1 : 7), defaults.CRC_POLY);
