...
function p = pset_new(id, name, cfreq, dbwidth, params)
    ...
    p.name = name;
    % Chip: frequency (Hz).  
    p.chip_frq = round(dbwidth / (defaults.CHIP_NFRQ * defaults.ALPHABET_SIZE));
    % Chip: duration (s).
    p.chip_dur = params.pset_chip_len_mul / p.chip_frq;

    % Interim values.
    nblock = dbwidth / (p.chip_frq * defaults.ALPHABET_SIZE);
    [p.prim_q, p.prim_a] = primitive(fix(nblock));

    % Number of frequency blocks.
    p.nblock = p.prim_q - 1;
    % Available bandwidth.
    p.bwidth = p.nblock * p.chip_frq * defaults.ALPHABET_SIZE;
