...
%     state = {};
    ...
    coder.varsize("null");
    null=[];
    state.pkt_raw_bits = null;
    state.pkt_dec_bits = null;
    state.crg_raw_bits = null;
    state.crg_dec_bits = null;
    %placeholders for coder
    state.gamma = 0;
    state. speed = 0;
    state.process_up_to = 0;
    state.bit_prob = null;
    ...
    %% Search 32 chips.
    pkt_nchip = defaults.ALPHABET_SIZE * (defaults.PKT_MIN_NBIT + defaults.CONV_ENC_MEM);
    ...
    [pkt_bytes, state.pkt_dec_bits, state.pkt_raw_bits pkt_bit_prob] = demod(pset, signal, bband_fs, slots, defaults.PKT_MIN_NBIT, state.gamma);
