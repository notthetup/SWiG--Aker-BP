...
static real_T emlrt_marshallIn(const emlrtStack *sp, const mxArray *u, const
  emlrtMsgIdentifier *parentId);
static void emlrt_marshallIn(const emlrtStack *sp, const mxArray *signalIn,
  ...
  const char_T *identifier, coder::array<real_T, 2U> &y)
{
  ...
  thisId.fParent = nullptr;
  thisId.bParentIsCell = false;
  emlrt_marshallIn(sp, emlrtAlias(signalIn), &thisId, y);
  emlrtDestroyArray(&signalIn);
}
...
static const mxArray *emlrt_marshallOut(const emlrtStack *sp, const struct2_T *u)
{
  ...
  static const char_T *sv[20]{ "pset_id", "pset_name", "cfreq", "bwidth",
    "chip_frq", "chip_dur", "prim_q", "prim_a", "nblock", "pkt_idx", "crg_idx",
    "pkt_raw_bits", "pkt_dec_bits", "crg_raw_bits", "crg_dec_bits", "gamma",
    "speed", "process_up_to", "bit_prob", "after" };
  ...
  k_y = nullptr;
  m = emlrtCreateDoubleScalar(u->pkt_idx);
  emlrtAssign(&k_y, m);
  emlrtSetFieldR2017b(y, 0, "pkt_idx", k_y, 9);
  l_y = nullptr;
  m = emlrtCreateDoubleScalar(u->crg_idx);
  emlrtAssign(&l_y, m);
  emlrtSetFieldR2017b(y, 0, "crg_idx", l_y, 10);
  emlrtSetFieldR2017b(y, 0, "pkt_raw_bits", emlrt_marshallOut
                      (u->pkt_raw_bits.data, u->pkt_raw_bits.size), 11);
  m_y = nullptr;
  iv1[0] = u->pkt_dec_bits.size[0];
  iv1[1] = u->pkt_dec_bits.size[1];
  m = emlrtCreateNumericArray(2, &iv1[0], mxDOUBLE_CLASS, mxREAL);
  pData = emlrtMxGetPr(m);
  i = 0;
  b_i = 0;
  while (b_i < u->pkt_dec_bits.size[1]) {
    for (b_i = 0; b_i < u->pkt_dec_bits.size[0]; b_i++) {
      pData[i] = u->pkt_dec_bits.data[b_i];
      i++;
    }
  ...
  emlrtAssign(&m_y, m);
  emlrtSetFieldR2017b(y, 0, "pkt_dec_bits", m_y, 12);
  n_y = nullptr;
  iv1[0] = 0;
  iv1[1] = 0;
  m = emlrtCreateNumericArray(2, &iv1[0], mxDOUBLE_CLASS, mxREAL);
  emlrtMxGetPr(m);
  emlrtAssign(&n_y, m);
  emlrtSetFieldR2017b(y, 0, "crg_raw_bits", n_y, 13);
  o_y = nullptr;
  iv1[0] = 0;
  iv1[1] = 0;
  m = emlrtCreateNumericArray(2, &iv1[0], mxDOUBLE_CLASS, mxREAL);
  emlrtMxGetPr(m);
  emlrtAssign(&o_y, m);
  emlrtSetFieldR2017b(y, 0, "crg_dec_bits", o_y, 14);
  p_y = nullptr;
  m = emlrtCreateDoubleScalar(u->gamma);
  emlrtAssign(&p_y, m);
  emlrtSetFieldR2017b(y, 0, "gamma", p_y, 15);
  q_y = nullptr;
  m = emlrtCreateDoubleScalar(u->speed);
  emlrtAssign(&q_y, m);
  emlrtSetFieldR2017b(y, 0, "speed", q_y, 16);
  r_y = nullptr;
  m = emlrtCreateDoubleScalar(u->process_up_to);
  emlrtAssign(&r_y, m);
  emlrtSetFieldR2017b(y, 0, "process_up_to", r_y, 17);
  emlrtSetFieldR2017b(y, 0, "bit_prob", emlrt_marshallOut(u->bit_prob.data,
    u->bit_prob.size), 18);
  s_y = nullptr;
  m = emlrtCreateDoubleScalar(u->after);
  emlrtAssign(&s_y, m);
  emlrtSetFieldR2017b(y, 0, "after", s_y, 19);
  return y;
}
...
void swig_rx_api(const mxArray * const prhs[4], int32_T nlhs, const mxArray
                 *plhs[2])
{
  coder::array<real_T, 2U> signalIn;
  ...
  signalIn.no_free();
  emlrt_marshallIn(&st, emlrtAlias(prhs[3]), "signalIn", signalIn);

  // Invoke the target function
  swig_rx(Rate_value, Fc_value, &params, signalIn, &pkt, &state);

  // Marshall function outputs
  plhs[0] = emlrt_marshallOut(&st, &pkt);
  if (nlhs > 1) {
    plhs[1] = emlrt_marshallOut(&st, &state);
  }
...
void swig_rx_atexit()
{
  ...
  swig_rx_xil_terminate();
  swig_rx_xil_shutdown();
...
// End of code generation (_coder_swig_rx_api.cpp)
