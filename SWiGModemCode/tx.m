...
%     state = {};
    ...
    trellis = poly2trellis(defaults.CONV_ENC_CLEN, defaults.CONV_ENC_CGEN);
    ...
    if (params.wut)
        wut = wake_up_tones(bband_ts, chip_nsample * defaults.NCHIP_WUT, pset.bwidth);
        wut_gap = zeros(fix(defaults.WUT_GAP * bband_fs), 1);
    else
        wut = complex(zeros(0,1));
        wut_gap = complex(zeros(0,1));
    end
    ...
    pkt_nchip = defaults.ALPHABET_SIZE * (pkt_nbits + defaults.CONV_ENC_MEM);
    pkt_conv = convenc([pkt_bits, zeros(1, defaults.CONV_ENC_MEM)], trellis);
    ...
    crg_nchip = 0;
    if (crg_nbits > 0)
        crg_nchip = defaults.ALPHABET_SIZE * (crg_nbits + defaults.CONV_ENC_MEM);
        crg_conv = convenc([crg_bits, zeros(1, defaults.CONV_ENC_MEM)], trellis);
    ...
    fh = (fix(defaults.CHIP_NFRQ / 2) * defaults.ALPHABET_SIZE + 1) * pset.chip_frq;
    bband = complex(zeros((ceil(nchip  * pset.chip_dur * bband_fs + 1)), 1));
    ...
    for kt = 1 : nchip
        ...
        fblock = - fh + slots(kt, 1) * defaults.ALPHABET_SIZE * pset.chip_frq;
    ...
    % Padding.
    if (params.pad)
        pad = zeros(defaults.PAD_NCHIP * chip_nsample, 1);
    else
        pad = zeros(0,1);
    end
